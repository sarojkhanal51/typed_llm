...
class _HomePageState extends State<_HomePage> {
  ...
  LlmProvider _buildProvider(String apiKey) {
    return switch (_provider) {
      _ProviderKind.openAi => OpenAiProvider(
        apiKey: apiKey,
      ...
      _ProviderKind.claude => ClaudeProvider(
        apiKey: apiKey,
  ...
  Future<void> _extract() async {
    ...
    final extractor = Extractor(
      provider: _buildProvider(_apiKeyController.text.trim()),
    );
    try {
        ...
        child: ListView(
          children: [
            SegmentedButton<_ProviderKind>(
              segments: const [
                ButtonSegment(
                  value: _ProviderKind.openAi,
                  label: Text('OpenAI'),
                ),
                ButtonSegment(
                  value: _ProviderKind.gemini,
                  label: Text('Gemini'),
                ),
                ButtonSegment(
                  value: _ProviderKind.claude,
                  label: Text('Claude'),
                ),
              ],
              selected: {_provider},
