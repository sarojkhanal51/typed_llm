...
> **Naming note:** `$Invoice` is generated by `typed_llm_generator` (see
> [Setup](#setup) below) — it doesn't exist until you run `build_runner`. It
> bundles the JSON Schema with the factory that rebuilds `Invoice`, which is
> what lets `extract` infer the return type and makes it impossible to pair
> one class's schema with another's parser.
